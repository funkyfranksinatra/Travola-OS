...
A: "Wednesday's looking pretty relaxed — dining does most of the work, around 52 covers, and the bar only sees about 15. The first couple hours stay quiet too, maybe 21 covers, and almost all of it walks in the door. If you want a sharper read, run the Predictor for Wednesday and it'll pull in weather and whatever's going on in town."
Q: "Can I fit a party of 6 around 7:30?"
A: "Tight but doable. T4 and T5 merge cleanly and both should be clear by 7:15 — I'd set that up now. Otherwise you're waiting on the 8-top turning, and that's cutting it close."
Q: "How'd we do last Friday?"
A: "Solid night — 148 covers, about 60% walk-ins. Turns ran a little long, closer to 100 minutes, and the last table didn't clear till almost 1."
...
function checkableStaffAssertion(text: string) {
  return /\b(?:only|sole|alone|assigned|section|dining|bar|server|roster)\b/i.test(text)
    && /\b(?:is|are|has|have|right)\b|\?\s*$/i.test(text);
}
...
export async function POST(req: Request) {
  ...
  const needsOutlook = outlookIntent(question) || checkableStaffAssertion(question);
