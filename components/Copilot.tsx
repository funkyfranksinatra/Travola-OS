...
export default function Copilot({ suppressed = false, floorEvent = 0 }: { suppressed?: boolean; floorEvent?: number }) {
  ...
  const send = useCallback(async (raw: string) => {
    ...
    try {
      const response = await fetch('/api/copilot/chat', {
        ...
        body: JSON.stringify({ messages: [...messages, user].map(({ role, content: message }) => ({ role, content: message })) }),
      });
  ...
  return <>
    ...
    <button onClick={() => { if (hasAlert && alert) openWithAlert(); else { setOpen(true); void runSentry(); } }} aria-label="Open co-pilot" title="Co-pilot" className="fixed bottom-4 right-4 z-40 flex h-12 w-12 items-center justify-center rounded-full border border-ai/70 bg-panel-card font-display text-xl text-ai shadow-[0_0_24px_rgba(139,139,255,.35)] transition hover:bg-panel-up hover:shadow-[0_0_30px_rgba(139,139,255,.55)]">
      <span aria-hidden="true">✦</span>{hasAlert && <span className="absolute right-1 top-1 h-2.5 w-2.5 rounded-full border-2 border-panel-card bg-amber-400" />}
    </button>
    {open && <aside className="fixed inset-y-0 right-0 z-40 flex w-[min(390px,100vw)] flex-col border-l border-border bg-panel shadow-2xl animate-[mesa-rail-in_0.2s_ease-out]" aria-label="Co-pilot">
      ...
      <div ref={threadRef} className="flex-1 overflow-y-auto p-4">
        ...
        <div className="space-y-3">{messages.map((message) => <div key={message.id} className={message.role === 'user' ? 'ml-8 rounded-lg bg-ai/15 px-3 py-2 font-mono text-[11px] leading-relaxed text-ink-100' : 'mr-4 rounded-lg border border-border bg-panel-card px-3 py-2 font-mono text-[11px] leading-relaxed whitespace-pre-wrap text-ink-200'}>{message.content}</div>)}</div>
      ...
      <form onSubmit={(event: FormEvent) => { event.preventDefault(); void send(draft); }} className="border-t border-border p-3">
        <div className="flex gap-2"><input value={draft} onChange={(event) => setDraft(event.target.value)} disabled={loading} placeholder="Ask co-pilot…" className="min-w-0 flex-1 rounded-md border border-border-hi bg-panel-card px-3 py-2 font-mono text-[11px] text-ink-50 outline-none placeholder:text-ink-400 focus:border-ai" /><button disabled={loading || !draft.trim()} className="rounded-md bg-ai px-3 font-mono text-[10px] font-bold uppercase tracking-[.1em] text-bg disabled:opacity-40">Send</button></div>
      </form>
    </aside>}
  </>;
}
