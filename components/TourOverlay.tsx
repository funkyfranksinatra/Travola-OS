...
export function TourOverlay({ steps, active = true, onComplete, onStepChange, onSkip, onAdvance, label = 'setup' }: {
  ...
  useEffect(() => {
    const handler = (event: Event) => {
      ...
      // State completion is authoritative. An event emitted from the same
      // state change would otherwise advance twice in one React commit.
      if (step?.advanceOn === `event:${name}` && step.complete !== true) { setDismissed(false); advance(); }
    };
